...
def dense_series() -> List[Tuple[str, str, str, str]]:
    return [
        ("original", "Original Greedy", "tab:blue", "o"),
        ("original_greedy_normal_v1", "Original Greedy Normal v1", "tab:cyan", "P"),
        ("original_greedy_normal_v2", "Original Greedy Normal v2", "tab:pink", "X"),
        ("matched_fixed_embb", "Matched Greedy", "tab:green", "^"),
        ("channel_only_greedy", "Channel-only Greedy", "tab:purple", "D"),
        ("rl", "SR-MAPPO", "tab:orange", "s"),
        ("embb_only_ceiling", "eMBB-only ceiling", "black", "x"),
        ("throughput_feasible_oracle", "Throughput-feasible oracle", "tab:red", "*"),
    ]
...
def normalize_dense_record(record: Dict, num_uavs: int) -> Dict[str, float]:
    ...
    reliability = float(record.get("urllc_reliability", record.get("urllc_success_rate", 0.0)))
...
_DENSE_METRIC_KEYS = [
    "embb_rate", "urllc_admission", "urllc_reliability", "total_power", "embb_power", "urllc_power",
    ...
    "overlay_feasible_pairs", "overlay_selected_pairs", "scheduled_packets", "scheduled_packets_per_uav",
]
...
def build_method_dense_summary(
    ...
    for load, records in zip(loads, records_by_load):
        ...
        load_entry.update({
            ...
            "overlay_selection_ratio": float(load_entry["overlay_ratio_stats"]["mean"]),
            "puncture_selection_ratio": float(load_entry["puncture_ratio_stats"]["mean"]),
        })
...
def nearest_replica_proxy(dense_bundle: Dict[str, Dict]) -> Dict[str, object]:
    compare_methods = ["original", "original_greedy_normal_v2", "matched_fixed_embb", "channel_only_greedy", "rl"]
...
def _plot_dense_band(ax, dense_bundle: Dict[str, Dict], metric_key: str, transform, ylabel: str, title: str, style_fn, top_axis_fn):
    ...
    for method_key, label, color, marker in dense_series():
        payload = dense_bundle.get(method_key)
        if payload is None:
            continue
...
def plot_matched_admission_diagnostics(matched_bundle: Dict[str, object], results_dir, style_fn):
    ...
    axes[0].plot(loads, matched_bundle["target_admission"], linestyle="--", color="black", label="Target admission")
    axes[1].plot(loads, matched_bundle["target_throughput"], linestyle="--", color="black", label="Target throughput (Mbps)")
...
def plot_low_damage_admission_diagnostics(low_damage: Dict, results_dir, style_fn):
    ...
    axes[0, 0].plot(loads, np.asarray(low_damage["loadwise_selected_puncture_loss"]) / 1e6, marker="o", label="SR-MAPPO")
    axes[0, 0].plot(loads, np.asarray(low_damage["baseline_puncture_loss"]) / 1e6, marker="x", linestyle="--", label="Original Greedy Normal v2")
    style_fn(axes[0, 0], "Selected puncture loss", "Average UE load per UAV", "Mbps loss/action")
    axes[0, 0].legend(fontsize=8)
    axes[0, 1].plot(loads, low_damage["loadwise_selected_overlay_retention"], marker="o", label="SR-MAPPO")
    axes[0, 1].plot(loads, low_damage["baseline_overlay_retention"], marker="x", linestyle="--", label="Original Greedy Normal v2")
    ...
    axes[0, 2].axhline(1.0, linestyle="--", color="black")
